...
O seguite diagrama ilustra o processo de recolha de dados e as decisões a tomar em cada etapa.

```{mermaid}
graph TD
    A[1. Selecionar o tipo de dados correto] --> B[2. Determinar o período de tempo]
    B -->|Recolher novos dados?| C[3.1 Decidir como os dados serão recolhidos]
    B -->|Utilizar dados existentes?| D[3.2 Escolher fontes de dados]
    C --> E[4.1 Decidir quantos dados a recolher]
    D --> F[4.2 Decidir quais dados utilizar]
```
...
-   **Dados Tabulares/estruturadps** São dados organizados em linhas e colunas. Cada linha representa um registo e cada coluna representa um atributo.
...
O enviesamnto dos dados ou da amostra é um problema comum na recolha de dados primários. Pode ocorrer quando a amostra não é representativa da população ou quando os dados são recolhidos de forma tendenciosa (para obter um determindado resultado). Por exemplo: *amostra de conveniência*, *amostra de voluntários*, *amostra de amigos*, etc.

-   **Dados secundários** - Dados que já foram recolhidos por outra pessoa ou organização para um propósito diferente. Por exemplo, *bases de dados públicas*, *relatórios de mercado*, *estudos científicostida*, etc.
...
Os dados podem estar ogranizados de diferentes formas, dependendo do tipo de análise que pretendemos realizar. As duas formas mais comuns de organizar os dados são:
...
-   **Periodos de tempo** ($t=1,...T$) - Anos, trimestres, meses, etc.
...
Várias entidades ($i=1,...n$), várias variáveis ($x_1,x_2, x_3,x_j$) e vários periodos de tempo ($t=1,...T$)
...
-   Fonte: WDI
-   [4 países:]{.underline} Portugal, Espanha, França e Grécia
-   [20 anos:]{.underline} até 2023
-   [3 variáveis:]{.underline} `GDP (constant LCU)`, `Population, total` e `Exports of goods and services (constant LCU)`
-   [Frequência:]{.underline} anual

2.  Contruir um conjunto de dados em formato de Séries Temporais com as seguintes características:

-   [Fonte]{.underline}: Eurostat
-   [Variável]{.underline}: `Nights spent at tourist accommodation establishments by residents/non-residents`
-   [País]{.underline}: Portugal
-   [Horizonte Temporal]{.underline}: máximo disponível
-   [Frequência]{.underline}: mensal