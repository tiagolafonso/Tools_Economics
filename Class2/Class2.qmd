...
```{mermaid}
graph TD
    A[Selecionar o tipo de dados] --> B[Determinar o período de tempo]
    B -->|Recolher novos dados?| C[3.1 Como os dados serão recolhidos]
    B -->|Utilizar dados existentes?| D[3.2 Fontes de dados]
    C --> E[4.1 Quantos dados a recolher]
    D --> F[4.2 Quais dados utilizar]
```
